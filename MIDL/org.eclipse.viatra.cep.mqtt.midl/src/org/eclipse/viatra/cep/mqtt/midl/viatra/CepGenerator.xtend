...
class CepGenerator {
	...
	def generateDashboardUtil(MqttSetup setup) {
		...
		public class DashboardUtil {
			...
			private static final Logger log4jLogger = Logger.getLogger(DashboardPublisher.class);
		
			private static final LoggerUtil LOGGER = new LoggerUtil(log4jLogger);
		
			private static Publisher publisher = new Publisher(«setup.brokerUrl», "IOT_CEP_DASHBOARD");;
		
			public static void publish(String eventId, String eventType) {
				try {
					...
					event.add("event_id", eventId);
					event.add("event_type", eventType);
